...
class DashboardController
{
    ...
    {
        try {
            // Vérifie que l'utilisateur est connecté et gère le timeout
            SessionManager::requireLogin();
            
            // Récupérer les données directement (sans cache)
            $vehicles = $this->vehicleRepository->findAll();
            $recentInterventions = $this->interventionRepository->findRecent(5);
            $allInterventions = $this->interventionRepository->findAll();
            ...
            // Véhicules nécessitant une attention (maintenance + hors service)
            $vehiclesNeedingAttention = $maintenanceVehicles + $outOfServiceVehicles;
            
            // Enrichir les données des véhicules avec emojis et classes CSS
            ...
            // Traductions françaises pour statuts et priorités
            $statusTranslations = [
                'pending' => 'En attente',
